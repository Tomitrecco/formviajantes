...
let currentRol = null; // 🔹 para guardar el rol del usuario
...
function mapRespuestasAPlanilla(r) {
  return {
    id: null, // si tenés un ID propio, setear acá
    gira_visita_id: r.gira_visita_id ?? null,

    // cliente existente
    tipo_gira: r.tipo_gira ?? null,
    ...
    propuesta_negocio: r.propuesta_negocio ?? null, // boolean o texto, tu doPost lo acepta
  ...
  if (!data || !data.length) {
    ...
    data.forEach(v => {
      ...
      card.querySelector('.cerrarBtn').addEventListener('click', async () => {
        ...
        // 🔹 NUEVO: enviar a Sheets
        const payloadSheets = mapRespuestasAPlanilla(insert);
        enviarFilaAGoogleSheets(payloadSheets);
...
function crearVisitaCard(giraId) {
  ...
  // Guardar visita (nueva), crear registros y enviar a Sheets
  card.querySelector('.cerrarBtn').addEventListener('click', async () => {
    const f = card.querySelector('.visitaForm');
    ...
    // 🔹 NUEVO: enviar a Sheets
    const payloadSheets = mapRespuestasAPlanilla(respuestasObj);
    enviarFilaAGoogleSheets(payloadSheets);
...
async function guardarRegistroVisitas() {
  ...
  if (error) {
    ...
    // 🔹 NUEVO: enviar cada fila a Sheets
    for (const r of inserts) {
      const payloadSheets = mapRespuestasAPlanilla(r);
      enviarFilaAGoogleSheets(payloadSheets);
    }
  }
}
